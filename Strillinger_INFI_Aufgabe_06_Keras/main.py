...
for i in range(0,100):
    ...
    path = os.path.join('zala', categories[real])
...
#------------------------------------------------------------------------------------------------
#3.3. Optimierung
model.save('zala/models/model.mdl')
model.save_weights("zala/models/model.h5")

weights = model.get_weights()
j =json.dumps(pd.Series(weights).to_json(orient='values'), indent=3)
#print(j)

model = keras.models.load_model('zala/models/model.mdl')
model.load_weights("zala/models/model.h5")

model_json = model.to_json()
print (model_json)

#------------------------------------------------------------------------------------------------
# 4 Veranschaulichung der Prediction
pd.DataFrame(prediction_dict).plot(figsize=(8,5))
plt.show()