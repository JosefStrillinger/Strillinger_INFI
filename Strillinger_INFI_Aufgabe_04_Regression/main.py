...
def predict_growth(y, v, year):
    ...
    #    avg_growth_per_year.append(v[i+1-v[i]])
    #avg_groth = sum(avg_growth_per_year)/len(avg_growth_per_year)
    #return str((year - y[0]) * avg_groth + v[0])
...
# 3
print("Prognose Angerberg 2030: " + predict_growth(pd.to_numeric(df.columns[3:]), pd.to_numeric(df.values[136, 3:]), 2030))
plt.plot(np.arange(2030, 2100), get_model(pd.to_numeric(df.columns[3:]), pd.to_numeric(df.values[136, 3:])).predict(pd.DataFrame({"years": np.arange(2030, 2100)})))
plt.title("Prognose Angerberg 2030 - 2100")
plt.savefig("img/3")
plt.show()
